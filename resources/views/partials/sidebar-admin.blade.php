<aside class="w-64 bg-gradient-to-b from-slate-800 to-slate-900 text-slate-100 min-h-screen flex flex-col">
    ...
    <div class="flex items-center gap-3 px-6 py-5 border-b border-slate-700/60">
        ...
        <div class="leading-tight">
            ...
            <span class="block text-xs text-indigo-300 font-medium">Admin Portal</span>
        </div>
    </div>
    ...
    <nav class="flex-1 px-3 py-5 space-y-0.5">
        ...
        <a href="{{ route('admin.dashboard') }}"
           class="sidebar-link {{ request()->routeIs('admin.dashboard') ? 'active' : '' }}">
            <svg class="w-5 h-5 {{ request()->routeIs('admin.dashboard') ? 'text-indigo-300' : 'text-slate-400' }}"
        ...
        <a href="{{ route('jobs.index') }}"
           class="sidebar-link {{ request()->routeIs('jobs.*') ? 'active' : '' }}">
            <svg class="w-5 h-5 {{ request()->routeIs('jobs.*') ? 'text-indigo-300' : 'text-slate-400' }}"
                 fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <a href="{{ route('admin.applications') }}"
           class="sidebar-link {{ request()->routeIs('admin.applications') ? 'active' : '' }}">
            <svg class="w-5 h-5 {{ request()->routeIs('admin.applications') ? 'text-indigo-300' : 'text-slate-400' }}"
                 fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round"
            ...
            @if(($pendingApplicationsCount ?? 0) > 0)
                <span class="inline-flex items-center justify-center w-5 h-5 text-[11px] font-bold bg-red-500 text-white rounded-full">
