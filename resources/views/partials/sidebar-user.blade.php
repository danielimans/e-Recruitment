<aside class="w-64 bg-gradient-to-b from-slate-800 to-slate-900 text-slate-100 min-h-screen flex flex-col">
    ...
    <div class="flex items-center gap-3 px-6 py-5 border-b border-slate-700">
        {{-- Logo Icon --}}
        <div class="flex items-center justify-center w-10 h-10 rounded-lg bg-indigo-600 text-white shadow">
    ...
    <nav class="flex-1 px-4 py-6 space-y-1">
        ...
        <a href="{{ route('user.applications') }}"
            ...
            <svg class="w-5 h-5 text-indigo-400" fill="none" stroke="currentColor" stroke-width="1.5"
                viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M8 7h8M8 11h8M8 15h8M4 6h16v12H4z" />
            </svg>
            My Applications
        </a>

        {{-- 4. Resume Builder (Moved Here) --}}
        <a href="{{ route('user.resume') }}"
            class="flex items-center gap-3 px-4 py-2 rounded-lg transition-colors duration-200 
            {{ request()->routeIs('user.resume') ? 'bg-indigo-600 text-white shadow-md' : 'hover:bg-slate-700' }}">
            {{-- Using Material Symbol to match your preference, colored to match theme --}}
            <span
                class="material-symbols-outlined text-[20px] {{ request()->routeIs('user.resume') ? 'text-white' : 'text-indigo-400' }}">description</span>
            Resume Builder
        </a>
    ...
    <div class="px-4 py-4 border-t border-slate-700">
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button
                class="w-full flex items-center gap-3 px-4 py-2 rounded-lg bg-red-600 hover:bg-red-700 transition text-white">
